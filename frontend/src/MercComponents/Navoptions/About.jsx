...
const About = () => {
  ...
  >
      ...
      <style>{`
        ...
        .back-btn { display:flex; align-items:center; gap:10px; background:transparent; border:1px solid rgba(255,255,255,0.08); border-radius:999px; padding:10px 20px 10px 14px; cursor:pointer; color:rgba(255,255,255,0.4); font-family:'JetBrains Mono',monospace; font-size:8px; letter-spacing:0.25em; text-transform:uppercase; transition:all 200ms; }
        .back-btn:hover { border-color:rgba(236,91,19,0.5); color:#ec5b13; }
        .slide-dot { height:2px; border:none; cursor:pointer; padding:0; border-radius:2px; transition:all 0.4s ease; }
        /* Desktop: 70vh; tablet: 65vh; mobile unchanged */
        .abt-hero { height: 70vh; min-height: 540px; }
...
<div className="abt-hero" style={{ position:"relative", overflow:"hidden" }}>
        {slides.map((s, i) => (
          <div key={i} style={{ position:"absolute", inset:0, opacity:i===currentSlide?1:0, transition:"opacity 1.2s ease", zIndex:i===currentSlide?1:0 }}>
            <img
              src={s.url}
              alt={s.label}
              style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"center top", filter:"grayscale(20%) contrast(1.08)", transform:`translateY(${scrollY*0.15}px)` }}
            />
          </div>
        ))}
        {/* Overlays */}
        ...
        <div style={{ position:"absolute", top: NAVBAR_H + 16, left:36, zIndex:10 }}>
        ...
        {/* Hero copy — pushed well below navbar */}
      ...
      <main style={{ maxWidth:1200, margin:"0 auto", padding:"88px 48px 120px" }}>
        ...
        <div className="abt-grid" style={{ display:"grid", gridTemplateColumns:"1.1fr 0.9fr", gap:72, alignItems:"center", marginBottom:120 }}>
          ...
          {/* ── Side image card — Premium badge repositioned inside ── */}
          <div className="abt-in" style={{ animationDelay:"0.3s", position:"relative" }}>
            <div style={{ borderRadius:24, overflow:"hidden", aspectRatio:"3/4", border:"1px solid rgba(255,255,255,0.06)", position:"relative" }}>
              ...
              {/* Premium badge — inside image, top-right corner, always visible */}
              <div style={{
                position:"absolute",
                top:16,
                right:16,
                ...
                color:"#000",
                boxShadow:"0 8px 24px rgba(236,91,19,0.45)"
              }}>
                Premium
              </div>
            </div>
        ...
        <div className="abt-in abt-stats" style={{ animationDelay:"0.35s", display:"grid", gridTemplateColumns:"repeat(4,1fr)", background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:24, overflow:"hidden", marginBottom:120 }}>
          {stats.map((s,i) => (
            <div
              key={i}
              style={{ padding:"44px 24px", textAlign:"center", borderRight:i<stats.length-1?"1px solid rgba(255,255,255,0.05)":"none", transition:"background 0.3s" }}
