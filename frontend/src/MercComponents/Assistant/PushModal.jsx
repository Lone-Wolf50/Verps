...
const PushModal = ({ session, type, onClose, onDone }) => {
	...
	return (
		<div style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(12px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
			<div style={{ background: "var(--bg-main)", border: type === "full" ? "1px solid rgba(239,68,68,0.3)" : T.border, borderRadius: 24, padding: "32px 28px", width: "100%", maxWidth: 480, animation: "slideInR 0.3s cubic-bezier(0.16,1,0.3,1) both" }}>
				{/* Title row */}
				<div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
					...
					<div>
						<h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, fontStyle: "italic", color: "var(--text-primary)", lineHeight: 1 }}>
				...
				<div style={{ display: "flex", gap: 10 }}>
					<button onClick={submit} disabled={sending || !reason.trim()} style={{ flex: 1, padding: "13px", cursor: "pointer", border: "none", borderRadius: 12, background: accentColor, color: "var(--text-primary)", fontFamily: "'DM Sans',sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", transition: "all 200ms", opacity: !reason.trim() || sending ? 0.4 : 1 }}>
