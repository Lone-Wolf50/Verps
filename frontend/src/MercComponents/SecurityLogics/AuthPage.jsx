...
const AuthPage_OtpForm = ({ onSuccess }) => {
  ...
  const verify = async (e) => {
    ...
    // ✅ FIX: Always trim both sides. Cast to string to avoid any type mismatch.
    const stored = String(localStorage.getItem("pendingOtp") || "").trim();
    let valid = stored.length === 6 && entered === stored;

    // Fallback: check DB (used for RESET flow where OTP is saved to DB)
    if (!valid) {
      ...
      if (dbUser?.otp_code) {
        ...
        // ✅ FIX: Don't check expiry for SIGNUP — only check for RESET where DB is used
        if (purpose === "RESET") {
...
const AuthPage = ({ mode: propMode }) => {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;1,400&family=JetBrains+Mono:wght@400;600;700&family=DM+Sans:ital,wght@0,400;0,500;0,600;0,700;0,800;1,400&display=swap');
